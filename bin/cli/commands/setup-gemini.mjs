...
export async function runSetupGeminiCommand(opts = {}) {
  ...
  const configPath =
    opts.configPath ?? opts["config-path"] ?? join(os.homedir(), ".gemini", "settings.json");
...
export function registerSetupGemini(program) {
  program
    ...
    .description(
      "Point the Gemini CLI at OmniRoute's native Gemini /v1beta endpoint (env recipe + settings model)"
    )
