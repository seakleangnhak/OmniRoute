...
async function runSession(input: RunInput, fetchImpl: FetchFn): Promise<TranslateNarratedResult> {
  ...
  if (mode === "send") {
    ...
    if (!sendRes.ok) {
      const errorBody = (await sendRes
        .json()
        .catch(() => ({ error: `HTTP ${sendRes.status}` }))) as { error?: unknown };
    ...
    const fetchMock: FetchFn = async (url, init) => {
      ...
      if (url.includes("translate")) {
        ...
        if (b.targetFormat === "openai") {
          return new Response(JSON.stringify({ success: true, result: { intermediate: true } }), {
            status: 200,
          });
        }
        return new Response(JSON.stringify({ success: true, result: { gemini: true } }), {
          status: 200,
        });
      }
      throw new Error(`Unexpected fetch: ${url}`);
...
describe("mode=preview, openai → claude (direct)", () => {
  it("calls translate once, pipelinePath=direct", async () => {
    const fetchMock: FetchFn = async (url, init) => {
      ...
      if (url.includes("translate")) {
        return new Response(JSON.stringify({ success: true, result: { claude: true } }), {
          status: 200,
        });
      }
      throw new Error(`Unexpected fetch: ${url}`);
...
describe("mode=send happy path", () => {
  it("calls detect + translate + send; status=ok, responsePreview populated", async () => {
    const fetchMock: FetchFn = async (url, init) => {
      ...
      if (url.includes("translate")) {
        return new Response(JSON.stringify({ success: true, result: { openai: true } }), {
          status: 200,
        });
      }
...
describe("error path — sanitization", () => {
  it("error message does not contain stack trace ('at /')", async () => {
    const fakeStack =
      "Translate failed at /home/user/foo.ts:42:10 sk-abcdefghijklmnopqrstuvwxyz1234567890";
  ...
  it("error with Bearer token is redacted", () => {
    ...
    assert.ok(
      ...
      `Token not redacted in: ${sanitized}`
    );
    assert.ok(
      sanitized.includes("Bearer [REDACTED]"),
      `Expected Bearer [REDACTED] in: ${sanitized}`
    );
  });
    ...
    const fetchMock: FetchFn = async (url, init) => {
      ...
      if (url.includes("translate")) {
        return new Response(
          JSON.stringify({
            success: false,
            error: "internal error at /home/user/foo.ts:42 sk-abcdefghijklmnopqrstuvwxyz1234567890",
          }),
          { status: 500 }
        );
      }
