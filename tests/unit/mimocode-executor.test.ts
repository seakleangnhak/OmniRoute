...
describe("mimocode per-account proxy", () => {
  ...
  it("syncAccountsFromCredentials replaces the default account with managed fingerprints", () => {
    const testExec = new MimocodeExecutor();
    ...
    assert.ok(
      !(testExec as any).accounts.some((account: any) => account.fingerprint === defaultFingerprint)
    );
  });

  it("syncAccountsFromCredentials removes stale fingerprints and restores the default", () => {
    const testExec = new MimocodeExecutor();
    const defaultFingerprint = (testExec as any).defaultFingerprint;

    (testExec as any).syncAccountsFromCredentials({
      providerSpecificData: { fingerprints: ["managed-fp-1", "managed-fp-2"] },
    });
    (testExec as any).syncAccountsFromCredentials({
      providerSpecificData: { fingerprints: ["managed-fp-2"] },
    });
    assert.deepStrictEqual(
      (testExec as any).accounts.map((account: any) => account.fingerprint),
      ["managed-fp-2"]
  ...
  it("rotates to the next account on a rate-limit-text 400 (#4976)", async () => {
    ...
    try {
      const result = await testExec.execute({
        ...
        signal: null,
        credentials: {
          providerSpecificData: { fingerprints: ["acct-a", "acct-b"] },
        },
        log: { debug: () => {}, info: () => {}, warn: () => {}, error: () => {} },
      });
