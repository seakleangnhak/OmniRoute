...
test("hideAvoid drops ToS-restricted rows from the table but keeps the count callout", () => {
  const shown = renderToStaticMarkup(
    React.createElement(FreeBudgetView, { data: layoutData, hideAvoid: false })
  );
  assert.match(tableOf(shown), /Kiro Auto/);
  const hidden = renderToStaticMarkup(
    React.createElement(FreeBudgetView, { data: layoutData, hideAvoid: true })
  );
...
test("sort=name orders the table rows alphabetically by display name", () => {
  const t = tableOf(
    renderToStaticMarkup(React.createElement(FreeBudgetView, { data: layoutData, sort: "name" }))
  );
