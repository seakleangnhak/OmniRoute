...
function clientRequest(url: string, init: RequestInit = {}): Request {
  const headers = new Headers(init.headers);
  if (!headers.has("authorization")) headers.set("authorization", `Bearer ${clientApiKey}`);
...
async function resetStorage() {
  ...
  const key = await apiKeysDb.createApiKey("Image route test client", "image-route-test-machine");
  clientApiKey = key.key;
}
...
test("v1 image generation POST accepts promptless requests for image-only models", async () => {
  ...
  const response = await imageRoute.POST(
    clientRequest("http://localhost/api/v1/images/generations", {
      method: "POST",
...
test("v1 image edit POST defers body-size validation to the provider", async () => {
  const response = await imageEditRoute.POST(
    clientRequest("http://localhost/api/v1/images/edits", {
      method: "POST",
      headers: {
...
test("v1 image edit POST keeps non-Codex providers single-reference", async () => {
  ...
  const response = await imageEditRoute.POST(
    clientRequest("http://localhost/api/v1/images/edits", { method: "POST", body: formData })
  );
  const body = (await response.json()) as ErrorResponseBody;
  assert.equal(response.status, 400);
...
test("provider-scoped image generation POST uses the shared 401 account fallback", async () => {
  ...
  const response = await providerImageRoute.POST(
    clientRequest("http://localhost/api/v1/providers/openai/images/generations", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
