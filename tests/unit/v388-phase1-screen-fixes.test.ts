...
test("search-tools: export modal mounts on exportOpen (not by default) without invalid isOpen prop", () => {
  ...
  assert.equal(
    /<ExportCodeModal[^>]*\bisOpen=/.test(src),
    false,
    "no invalid isOpen prop on ExportCodeModal"
  );
});
...
test("memory: tabs ordered memories -> engine -> playground", () => {
  ...
  assert.ok(
    src.includes('["memories", "engine", "playground"]'),
    "TABS order is memories, engine, playground"
  );
});
...
test("logs: proxy/console tabs removed (dedicated menu pages exist)", () => {
  ...
  assert.equal(
    src.includes("SegmentedControl"),
    false,
    "SegmentedControl removed (single tab left)"
  );
});
