...
function isSchemaAlreadyApplied(
  ...
  switch (migration.version) {
    ...
    case "068":
      return hasTable(db, "free_proxies");
    case "070":
    ...
    case "077":
      return hasColumn(db, "api_keys", "stream_default_mode");
    case "078":
      return hasTable(db, "quota_consumption");
    case "083":
      // Plan 21 D27 fix: guard memory_vec migration. Without this case, an
      // unmarked re-run of 083_memory_vec.sql would have its ALTER TABLE fail
...
function reconcileMissingFreeProxiesTable(db: SqliteAdapter): boolean {
  const applied = db
    .prepare("SELECT version FROM _omniroute_migrations WHERE version = ? AND name = ?")
    .get("068", "free_proxies") as { version: string } | undefined;

  if (!applied || hasTable(db, "free_proxies")) {
    return false;
  }
  ...
  console.warn(
    "[Migration] Removed stale 068_free_proxies marker because the physical table is missing."
  );
  return true;
}
...
export function runMigrations(db: SqliteAdapter, options?: { isNewDb?: boolean }): number {
  ...
  reconcileMissingFreeProxiesTable(db);
