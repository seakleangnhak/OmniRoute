...
export function LlmChatCard({
  ...
  const { models, loading, error, retry } = useProviderModels(providerId, connectionId);
          ...
          <div className="flex items-center gap-1.5 min-w-0 flex-1">
            ...
            {error && (
              <span className="text-xs text-red-500 flex items-center gap-1" role="alert">
                <span className="truncate max-w-[180px]" title={String(error)}>{String(error)}</span>
                <button type="button" onClick={retry} className="shrink-0 text-xs text-primary hover:text-primary-strong underline">
                  {t("retry") ?? "Retry"}
                </button>
              </span>
            )}
