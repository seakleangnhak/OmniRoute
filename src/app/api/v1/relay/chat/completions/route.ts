...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!rawToken) {
      return new Response(
        JSON.stringify({
          error: { message: "Missing relay token", type: "auth_error", code: "RELAY_AUTH_001" },
        }),
    ...
    if (!token) {
      ...
      return new Response(
        JSON.stringify({
          error: { message: "Invalid relay token", type: "auth_error", code: "RELAY_AUTH_002" },
        }),
        { status: 401, headers: { ...CORS_HEADERS, "Content-Type": "application/json" } }
      );
    }
    ...
    if (token.expiresAt && Math.floor(Date.now() / 1000) > token.expiresAt) {
      return new Response(
        JSON.stringify({
          error: { message: "Relay token expired", type: "auth_error", code: "RELAY_AUTH_003" },
        }),
        { status: 401, headers: { ...CORS_HEADERS, "Content-Type": "application/json" } }
      );
    }
    ...
    if (!rateCheck.allowed) {
      ...
      return new Response(
        JSON.stringify({
          error: { message: "Rate limit exceeded", type: "rate_limited", code: "RELAY_RATE_001" },
        }),
        {
          status: 429,
          headers: {
        ...
        if (allowedModels.length > 0 && !allowedModels.includes("*")) {
          ...
          if (!allowed) {
            return new Response(
              JSON.stringify({
                error: {
                  message: `Model "${model}" not allowed by this relay token`,
                  type: "model_not_allowed",
                  code: "RELAY_MODEL_001",
                },
              }),
    ...
    return new Response(
      JSON.stringify({
        error: { message: `Relay error: ${message}`, type: "relay_error", code: "RELAY_ERR_001" },
      }),
