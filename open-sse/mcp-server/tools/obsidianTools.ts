...
export const obsidianTools = [
  {
    ...
    description:
      "Check whether the Obsidian Local REST API is reachable and authenticated. Returns connection status.",
  ...
  },
  {
    ...
    description:
      "Search note content in Obsidian vault by text query. Returns matching snippets with file paths.",
    scopes: ["read:obsidian"],
    inputSchema: z.object({
      ...
      contextLength: z
        ...
        .max(500)
        .default(100)
        .describe("Characters of context around each match"),
    }),
  ...
  },
  {
    ...
    description:
      "Search Obsidian vault using a JSON Logic expression for complex queries (and, or, regex, path filters, etc.).",
    scopes: ["read:obsidian"],
    inputSchema: z.object({
  ...
  },
  {
    ...
    description:
      "Read the full content of a note in the Obsidian vault by its vault-relative path. Optionally target a specific heading or block.",
    scopes: ["read:obsidian"],
    inputSchema: z.object({
      ...
      targetType: z
        ...
        .optional()
        .describe("Scope the read to a specific type of target"),
      target: z
        .string()
        .optional()
        .describe(
          "Target identifier: heading text, block index (e.g., '^block-id'), or 'frontmatter'"
        ),
    }),
    handler: async (
      args: { path: string; targetType?: "heading" | "block" | "frontmatter"; target?: string },
      extra?: McpExtra
    ) => {
      const client = getClient(extractApiKeyId(extra));
  ...
  },
  {
    ...
    description:
      "List files and directories in the Obsidian vault. Returns a tree of file/folder entries.",
    scopes: ["read:obsidian"],
    inputSchema: z.object({
      path: z
        .string()
        .optional()
        .default("")
        .describe("Vault-relative directory path to list (empty for root)"),
    }),
  ...
  },
  {
    ...
    description:
      "Get the document structure of a note as a map of headings and their line numbers.",
    scopes: ["read:obsidian"],
    inputSchema: z.object({
  ...
  },
  {
    ...
    description:
      "Get metadata (frontmatter, tags, links, char/word count) for a note without reading its full content.",
    scopes: ["read:obsidian"],
    inputSchema: z.object({
      path: z.string().min(1).describe("Vault-relative path to the note"),
    }),
  ...
  },
  {
    ...
    description:
      "Get the daily, weekly, or monthly periodic note for a given date (or today if no date specified).",
    scopes: ["read:obsidian"],
    inputSchema: z.object({
    ...
    handler: async (
      args: { period: "daily" | "weekly" | "monthly"; year?: number; month?: number; day?: number },
      extra?: McpExtra
    ) => {
      const client = getClient(extractApiKeyId(extra));
  ...
  },
  {
    ...
    description:
      "List all available Obsidian commands with their IDs and names. Use IDs with obsidian_execute_command.",
    scopes: ["read:obsidian"],
    inputSchema: z.object({}),
    handler: async (_args: unknown, extra?: McpExtra) => {
      const client = getClient(extractApiKeyId(extra));
  ...
  },
  {
    ...
    description:
      "Create or overwrite a note in the Obsidian vault with the given markdown content.",
    scopes: ["write:obsidian"],
    inputSchema: z.object({
      path: z
        .string()
        .min(1)
        .describe("Vault-relative path to write to (e.g., 'notes/new-note.md')"),
  ...
  },
  {
    ...
    description:
      "Append content to an existing note. Optionally append to a specific heading or block.",
    scopes: ["write:obsidian"],
    inputSchema: z.object({
      path: z.string().min(1).describe("Vault-relative path to the note"),
      ...
      targetType: z
        .enum(["heading", "block", "frontmatter"])
        .optional()
        .describe("Target type to append to"),
      target: z
        .string()
        .optional()
        .describe("Target identifier (heading text, block index, or 'frontmatter')"),
    }),
    handler: async (
      args: {
        path: string;
  ...
  },
  {
    ...
    description:
      "Surgically patch a note — append, prepend, or replace content at a specific heading, block, or frontmatter field.",
    scopes: ["write:obsidian"],
    inputSchema: z.object({
      path: z.string().min(1).describe("Vault-relative path to the note"),
      ...
      target: z
        .string()
        .min(1)
        .describe("Target identifier: heading text, block ID, or frontmatter key"),
      content: z.string().min(1).describe("Content to apply with the patch operation"),
      createTargetIfMissing: z
        .boolean()
        .optional()
        .default(false)
        .describe("Create the target heading/block if it does not exist"),
    }),
    handler: async (
      args: {
        path: string;
  ...
  },
  {
    ...
    inputSchema: z.object({
      ...
      destination: z
        .string()
        .min(1)
        .describe("New vault-relative path (can include new parent directory)"),
    }),
  ...
  },
  {
    ...
    description:
      "Execute an Obsidian command by its command ID. Use obsidian_list_commands to discover available command IDs.",
    scopes: ["write:obsidian"],
    inputSchema: z.object({
  ...
  },
  {
    ...
    description:
      "Open a file in Obsidian. If the file exists it opens in the editor; if not, it creates a new file.",
    scopes: ["write:obsidian"],
    inputSchema: z.object({
  ...
  },
  {
    ...
    inputSchema: z.object({
      path: z
        .string()
        .min(1)
        .describe("Vault-relative path of the conflicting file (without .conflict- suffix)"),
      resolution: z
        .enum(["local", "remote", "keep-both"])
        .describe("Which version to keep: local (mobile), remote (desktop), or keep-both"),
    }),
    handler: async (
      args: { path: string; resolution: "local" | "remote" | "keep-both" },
      extra?: McpExtra
    ) => {
      const token = getSyncToken() || requireToken(extractApiKeyId(extra));
      const syncClient = createSyncServerClient(token);
