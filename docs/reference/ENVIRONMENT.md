...
| `OMNIROUTE_DISABLE_LIVE_WS`                 | `false`                         | `src/server/ws/liveServer.ts`                                            | Set to `1` or `true` to disable the real-time WebSocket server.                                                                                             |
...
| `OMNIROUTE_CGPT_WEB_IMAGE_CACHE_MAX_MB`    | `256`                                                           | `open-sse/services/chatgptImageCache.ts`    | Hot in-memory byte budget (MB) for the chatgpt-web image cache serving `/v1/chatgpt-web/image/<id>`. SQLite remains the persistent cache across restarts.                                                                                                                                         |
| `OMNIROUTE_CGPT_WEB_IMAGE_CACHE_DB_MAX_MB` | `1024`                                                          | `open-sse/services/chatgptImageCache.ts`    | Persistent SQLite byte budget (MB) for chatgpt-web image cache rows. Oldest rows are evicted after expired rows are removed.                                                                                                                                                                      |
| `OMNIROUTE_CGPT_WEB_IMAGE_CACHE_TTL_HOURS` | `168`                                                           | `open-sse/services/chatgptImageCache.ts`    | Persistent chatgpt-web image cache TTL in hours. Cache IDs and `/v1/chatgpt-web/image/<id>` URLs remain usable until this TTL expires or the persistent byte budget evicts them.                                                                                                                  |
...
| Variable                   | Default                          | Description                                                            |
| -------------------------- | -------------------------------- | ---------------------------------------------------------------------- |
| `OMNIROUTE_MEMORY_MB`      | `2048` (Docker) / system default | V8 heap limit. Sets `--max-old-space-size` for the server process.     |
